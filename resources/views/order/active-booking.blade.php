                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-header-form d-flex">
                                ...
                                <form action="/booking/active" method="GET">
                            ...
                            <div class="table-responsive">
                                <table class="table-striped table">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if (\Carbon\Carbon::parse($order->booking->start_date) <= now())
                                                Terselenggara
                                            @else
                                            Belum Terselenggara
                                            @endif
                                        ...
                                        <td class="d-flex gap-2" style="align-items:flex-start">
                                            <button type="button"
                                            onclick="orderDetail({{ $order }})"
                                            data-bs-toggle="modal" data-bs-target="#detailOrder"
                                            class="btn btn-secondary mr-2">Detail</button>
                                            
                                            @if (\Carbon\Carbon::parse($order->booking->start_date)->diffInDays(now()) < 1 && $order->booking->started_confirm_at == null)
                                            <form method="POST" action="/booking/package/start/{{ $order->booking->id }}">
                                                @method('put')
                                                @csrf
                                                <button onclick="return window.confirm('Yakin konfirmasi?') ? true : false" type="submit" class="btn btn-success mr-2">Konfirmasi</button>
                                            </form>
                                        @endif
                                            </td>
        ...
        const orderDetail = (order) => {
            ...
            let statusText = '';
             if (!order.reference || order.reference.status == 'pending') {
                ...
                let statusConfirmation = ''
                if(order.booking.started_confirm_at){
                    statusConfirmation = 'Terkonfirmasi'
                }else{
                    statusConfirmation = 'Belum terkonfirmasi'
                }
                ...
                let item = `
                    ...
                    <div class="modal-body">
                        <ul class="list-group">
                            ...
                            <li class="list-group-item d-flex align-items-center gap-2">
                                <span class="material-icons mr-2 fw-bold">Status konfirmasi: </span> ${statusConfirmation}
                            </li>
                            ${expiredText}
                        </ul>
                    </div>
